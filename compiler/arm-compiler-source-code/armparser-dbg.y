...
void setInstructionI(){ 
  current_instruction.replace(31 + (dbg? ++index_debug: 0),1,"1");

}
void setOp(std::string cmd){
  current_instruction.replace(0,4,cmd + bin_debug);
}


void setRK(std::string RK){
  current_instruction.replace( 4+ (dbg? ++index_debug: 0), 4,RK + bin_debug);
}



void setRd(std::string Register){
  current_instruction.replace( 4+ (dbg? ++index_debug: 0), 4,RegisterToBinary(Register)+ bin_debug);
}
void setRs(std::string Register){
  current_instruction.replace( 8+ (dbg? ++index_debug: 0), 4,RegisterToBinary(Register)+ bin_debug); 
}
void setRx(std::string Register){
  current_instruction.replace(12+ (dbg? ++index_debug: 0), 4,RegisterToBinary(Register)+ bin_debug); 
}
...
void insertIntOnInstruction(int imm_int){
  ...
  current_instruction.replace( 12+ (dbg? ++index_debug: 0), 19, imm_str + bin_debug);
}
...
void setImmDecLdr(std::string imm_dec){
  ...
  if (0 <= imm_int && imm_int <= 6){
    ...
    current_instruction.replace( 8+ (dbg? ++index_debug: 0), 4, imm_str+ bin_debug);
  }
  else {
    error_count++;
    printt("Error: Immediate value outbounds \"" + imm_dec + "\".");
...
void setBranchReference(int current_pos, int target_pos){
  ...
  current_instruction.replace( 4+ (dbg? ++index_debug: 0), 28, branch_target + bin_debug);
}
